...
public class UIText extends CoreWidget {
    ...
    public boolean onKeyEvent(NUIKeyEvent event) {
        ...
        if (isEnabled() && event.isDown() && lastFont != null) {
            if (isShowingHintText && !readOnly) {
                if ((event.getKeyboard().isKeyDown(Keyboard.KeyId.LEFT_CTRL) || event.getKeyboard().isKeyDown(Keyboard.KeyId.RIGHT_CTRL)) && event.getKey() == Keyboard.Key.V) {
                    removeSelection();
                    paste();
                    eventHandled = true;
                }
                ...
                switch (event.getKey().getId()) {
                    ...
                    default: {
                        if ((event.getKeyboard().isKeyDown(KeyId.LEFT_CTRL) || event.getKeyboard().isKeyDown(KeyId.RIGHT_CTRL)) && event.getKey() == Keyboard.Key.C ) {
                            copySelection();
                            eventHandled = true;
                            break;
                        }
                ...
                if (!readOnly) {
                    switch (event.getKey().getId()) {
                        ...
                        case KeyId.NUMPAD_ENTER: {
                            if ((event.getKeyboard().isKeyDown(Keyboard.Key.LEFT_SHIFT.getId()) || event.getKeyboard().isKeyDown(Keyboard.Key.RIGHT_SHIFT.getId())) && multiline ) {
                                setText(fullText + "\n");
